...
(defclass lazy-nd-array ()
  ((dimensions-of :initarg :dimensions-of :accessor dimensions-of) ; !
...
(defmethod dref ((obj lazy-nd-array) array d) ; ??  Recursive!
  (cond ((delayed-p (aref array (car d)))
        ...
        (t (dref obj (aref array (car d)) (cdr d)))) )
...
(defmethod (setf dref) (val (obj lazy-nd-array) array d)
  (cond ((delayed-p (aref array (car d)))
         (setf (aref array (car d))
               (force (aref array (car d)))) )
        ((null (cdr d))
...
(defun lazy-nd-init (vector n)
  (cond ((null n) vector)
        ...
        (t (setf (aref vector (- (car n) 1))
                 (delay (lazy-nd-init nil (cdr n))))
           (lazy-nd-init vector (cons (- (car n) 1) (cdr n)))) ))
